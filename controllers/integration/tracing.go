...
func (r *TracingReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	c, err := controller.NewUnmanaged("tracingintegration", options)
	if err != nil {
		return err
	}
	for _, watch := range []source.Source{
		...
		source.Kind[client.Object](mgr.GetCache(), &telemetryv1.Telemetry{}, telemetryHandler),
	} {
	...
	// RunnableFunc belongs to the manager's leader-election group by default.
	// Keep the wait and controller in the same managed goroutine so shutdown and
	// errors propagate normally. Kind sources only create informers on Start.
	return mgr.Add(manager.RunnableFunc(func(ctx context.Context) error {
		return startTracingController(ctx, mgr.GetCache(), c, logger)
	}))
}

func startTracingController(ctx context.Context, crdCache cache.Cache, c manager.Runnable, logger logr.Logger) error {
	logger.Info("Waiting for tracing integration CRDs")
	if err := waitForCRDs(ctx, crdCache, []string{
	...
	logger.Info("Tracing integration CRDs are ready; starting controller")
	return c.Start(ctx)
}

// waitForCRDs waits until all named CRDs are established, not terminating, and
// serving at least one version. It watches CRD objects, not custom resources.
// The shared informer handles list/watch retries and updates its store before
// notifying us, so readiness checks use the same cache as the event source.
func waitForCRDs(ctx context.Context, crdCache cache.Cache, names []string) error {
	if len(names) == 0 {
		return nil
	}
	...
	for {
		ready, err := crdsReady(ctx, crdCache, names)
		if err != nil {
			return err
		}
		if ready {
...
// crdsReady checks readiness of the named CRDs using the supplied reader.
func crdsReady(ctx context.Context, reader client.Reader, names []string) (bool, error) {
	for _, name := range names {
