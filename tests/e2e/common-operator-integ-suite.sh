...
ISTIO_NAME="${ISTIO_NAME}" COMMAND="${COMMAND}" VERSIONS_YAML_FILE="${VERSIONS_YAML_FILE}" KUBECONFIG="${KUBECONFIG}" ISTIOCTL_PATH="${ISTIOCTL}" SKIP_CLEANUP="${SKIP_CLEANUP}" \
go run github.com/onsi/ginkgo/v2/ginkgo -tags e2e --timeout 30m ${GINKGO_FLAGS} "${WD}"/...
